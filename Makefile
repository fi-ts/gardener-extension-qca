...
LD_FLAGS                    := "-w -X github.com/fi-ts/gardener-extension-qca/pkg/version.Version=$(IMAGE_TAG)"
VERIFY                      := true
...
.PHONY: build
build:
	go build -ldflags $(LD_FLAGS) -tags netgo -o bin/gardener-extension-qca ./cmd/gardener-extension-qca
...
docker-image:
	@docker build --no-cache \
		...
		--tag $(IMAGE_PREFIX)/gardener-extension-qca:$(IMAGE_TAG) \
		--file Dockerfile --memory 6g .

.PHONY: docker-push
docker-push:
	@docker push $(IMAGE_PREFIX)/gardener-extension-qca:$(IMAGE_TAG)
...
generate: $(VGOPATH) $(HELM) $(YQ)
	echo $(shell git -c safe.directory=/go/src/github.com/fi-ts/gardener-extension-qca describe --abbrev=0 --tags --always) > VERSION
...
generate-in-docker: tidy install $(HELM) $(YQ)
	docker run --rm -i$(DOCKER_TTY_ARG) -v $(PWD):/go/src/github.com/fi-ts/gardener-extension-qca golang:$(GO_VERSION) \
		sh -c "cd /go/src/github.com/fi-ts/gardener-extension-qca \
				&& make generate \
